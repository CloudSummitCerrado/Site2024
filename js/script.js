	...
	if($('.cs-countdown').length){
	   $(function(){
		   ...
		   $this.countdown(finalDate, function(event) {
		     $this.html(even.tstrftime('%D dias %H:%M:%S'));
		   });
		 });
		});

	   $('.cs-countdown').countdown('').on('update.countdown', function(event) {
		  var $this = $(this).html(event.strftime('<div><span>%D</span><h6>dias</h6></div> <div><span>%H</span><h6>horas</h6></div> <div><span>%M</span><h6>Minutos</h6></div> <div><span>%S</span><h6>Segundos</h6></div>'));
		});
	}
